...
function renderFeed(posts){
  ...
  for(const p of posts){
    ...
    // Actions (Profile, Translate, Like, Comment, Share)
...
/* WRITE */
elRegister.addEventListener("click", async ()=>{ // Register new user
  try{
    ...
    await Promise.all([refreshBalances(), refreshFeed()]);   // Re-render feed to display actions
  }catch(e){ console.error(e); elComposeMsg.textContent=`Register failed.`; alert("Register failed or rejected."); }
});

elPublish.addEventListener("click", async ()=>{ // Create new post
  try{
    if(!account) await connectWallet();
